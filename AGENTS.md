...
- Recipes without `extendBase`/`extends` memoize their output per selection key (`buildSelectionKey`, bounded FIFO cache of 512 in `shared.ts`; `test/resolve-cache.test.ts` pins the contract). `extendBase` is user code, so recipes using it opt out of memoization — keep that opt-out. The selection key must cover every name that can influence output: variant names, compound-condition keys (including runtime-only non-variant conditions), and `defaultVariants` keys (`collectSelectionKeyNames`).
